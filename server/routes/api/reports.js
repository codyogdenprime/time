  ...
  console.log(req.query,'admin query ');
    ...
    pg.connect(connectionString, function(err, client, done) {
        if (err) {
            ...
            var resultsArray = [];
            var queryResults = client.query('SELECT * FROM projects JOIN time on projectid = projid JOIN employee ON time.empid = employee.empid WHERE client_id = $1 AND time.date >= $2 AND time.date <= $3', [objectIn.client_id, objectIn.sDate, objectIn.eDate],function(err, response){
              if(err){
                console.log(err.message);
                done();
                res.send({success:false, error:err.message});
